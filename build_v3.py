...
OUT = r"C:\research-training\repro-ncf\NCF复现与暑期学习汇报_徐胜钦_v3.pptx"
...
def set_title(slide, text, size=26, color=DARK, align=PP_ALIGN.LEFT):
...
# ---------- S1 cover ----------
s = new_slide("title-cover")
add_bar(s, 0, 0, 13.333, 0.12, GOLD)                      # top gold line
add_bar(s, 0, 7.38, 13.333, 0.12, GOLD)                   # bottom gold line
fill_ph(s, 11, [[("论文复现与暑期学习汇报", 40, True, DARK)],
                [("NCF · Neural Collaborative Filtering (WWW 2017)", 17, False, GRAY)],
                [("MovieLens-1M 复现实验", 17, False, GRAY)]], align=PP_ALIGN.LEFT)
fill_ph(s, 15, [[("徐胜钦", 15, True, ACCENT)],
                [("重庆大学 · 大数据与软件学院", 12, False, GRAY)],
                [("2026级硕士研究生", 12, False, GRAY)]], align=PP_ALIGN.LEFT)
fill_ph(s, 14, [[("汇报时间:2026.09", 11, False, GRAY)]], align=PP_ALIGN.RIGHT)
...
set_title(s, "为什么复现 NCF")
card3_columns(s)
cards3 = [
    ("领域代表作", ["神经协同过滤奠基工作", "WWW 2017 · 被引数千", "理解「深度学习如何建模交互」的入口"]),
    ("结果可对照", ["论文报告 HR@10 / NDCG@10", "代码规模小,单卡可跑", "权威公开数字检验复现质量"]),
    ("为科研打底", ["官方 TF1 → 本复现 PyTorch", "与序列 / GNN 推荐一脉相承", "对接组内方向与后续研究"]),
]
...
add_textbox(s, 0.55, 6.55, 12.3, 0.6, [[("协议:隐式反馈二值化 · log loss · 训练负采样 1:4 · leave-one-out 测试 · HR@10 / NDCG@10", 11.5, False, GRAY)]], align=PP_ALIGN.CENTER)
...
setup3 = [
    ...
    ("协议与超参", ["测试:1 正 + 99 负 = 100 候选", "factor=8 · MLP 3 层 · batch 256 · lr 1e-3", "50 epochs · 逐 epoch 全量评估记录"]),
]
...
card3_columns(s)
prob3 = [
    ("数据真伪甄别", ["预下载「ml-1m」实为模拟数据", "行为序列异常(user1: item 1..35 连续)", "改取论文官方处理产物并校验行数", "教训:先核数据源,指标才有意义"]),
    ("工具链与兼容", ["RTX 5070 Ti(sm_120)需 cu128+", "预装 torch 为 CPU 版 → 强制重装 cu130", "GitHub / grouplens 不通 → 代理镜像取数"]),
    ("复现差距归因", ["TF→PyTorch 实现细节差异", "训练预算与调参策略不同", "Figure 读数 ±0.005 误差", "相对趋势一致 → 复现可信"]),
]
